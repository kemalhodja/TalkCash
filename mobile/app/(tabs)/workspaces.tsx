...
import { consumePendingWorkspaceInvite } from "@/hooks/useAssistantLinking";
...
type Invitation = { id: string; email: string; role: string; status: string; accept_url?: string };
...
export default function WorkspacesScreen() {
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      ...
      setPremium(status);
      const pending = await api.getWorkspaceInvitationInbox().catch(() => []);
      setInbox(pending);
  ...
  const acceptInvite = useCallback(async (token: string) => {
    ...
    try {
      ...
      Alert.alert(t.common.confirm, t.workspaces.inviteAccepted);
      await load(true);
    } catch (e: any) {
  ...
  }, [acceptingToken, load, t.common.confirm, t.common.error, t.workspaces.inviteAccepted]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const pending = await consumePendingWorkspaceInvite();

      const token = (typeof params.accept === "string" ? params.accept : "") || pending || "";

      if (token) await acceptInvite(token);

    })();

  }, [params.accept, acceptInvite]);
  ...
  const invite = async (workspaceId: string) => {
    ...
    try {
      ...
      const res = await api.inviteWorkspaceMember(workspaceId, email, inviteRole[workspaceId] || "member");
      ...
      if (res?.accept_url) {
        ...
        Share.share({
          ...
          message: t.workspaces.shareInviteMessage

            .replace("{name}", items.find((i) => i.id === workspaceId)?.name || "")

            .replace("{email}", email)

            .replace("{link}", res.accept_url),
  ...
  if (error && !locked && !inbox.length) return <ErrorState message={error} onRetry={() => load(true)} />;
  ...
  return (
    ...
    <ScreenShell ambient="subtle">
      ...
      {inbox.length > 0 ? (
        ...
        <Surface variant="accent" style={styles.card}>

          <Text style={styles.title}>{t.workspaces.inboxTitle}</Text>
          ...
          {inbox.map((inv) => (
            ...
            <View key={inv.id} style={styles.inboxRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.inboxName}>{inv.organization_name}</Text>

                <Text style={styles.meta}>{t.workspaces.inboxRole.replace("{role}", inv.role)}</Text>

              </View>

              <PrimaryButton

                label={t.workspaces.acceptInvite}
                ...
                <>
                  ...
                  <View style={styles.inviteBlock}>
                    ...
                    <PrimaryButton
                      ...
                      label={t.workspaces.shareInvite}
                      ...
                      onPress={() => {
                        ...
                        const email = (inviteEmail[item.id] || "").trim();

                        const pending = (invitations[item.id] || []).find((i) => i.email === email);
                        ...
                        Share.share({
                          ...
                          message: t.workspaces.shareInviteMessage
                            ...
                            .replace("{email}", email || t.workspaces.inviteEmail)

                            .replace("{link}", pending?.accept_url || ""),
...
const styles = StyleSheet.create({
  ...
  inboxRow: { flexDirection: "row", alignItems: "center", gap: Spacing.sm, marginBottom: Spacing.sm },

  inboxName: { color: Colors.text, fontWeight: "700" },
