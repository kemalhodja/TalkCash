...
import { RevenueCatBillingError } from "@/services/revenueCat";
...
export function PaywallCard({
  ...
  const upgrade = async () => {
    ...
    try {
      ...
      hapticImpact("error");
      if (e instanceof RevenueCatBillingError && e.userCancelled) return;
  ...
  const restore = async () => {
    ...
    try {
      ...
      Alert.alert(
        t.premium.title,
        message.includes("No active") ? t.premium.noSubscriptions : message,
      );
    } finally {
      setRestoring(false);
    }
